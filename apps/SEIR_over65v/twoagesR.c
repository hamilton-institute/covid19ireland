...
struct simresult seir_model_erlang(double , double , double , const struct compartments *, const struct compartments *, const double mean_holding_times[2], const double beta[2][2][2], const struct subcompartments *S, const struct subcompartments *, int *Yv, int Yvl, int *Ov, int Ovl);

void freeresult(struct simresult res);

SEXP twoages(SEXP YS, SEXP YE, SEXP YI, SEXP YR, SEXP OS, SEXP OE, SEXP OI, SEXP OR, SEXP YR0Y, SEXP YR0O, SEXP OR0Y, SEXP OR0O, SEXP Yvac, SEXP Ovac) {
	...
	for(int i = 0; i < N_phases; i++) {
		if (i == 0) { /* for first phase of the epimedic */
			realisation = seir_model_erlang(t_phase, t, dt, &Y, &O, mean_holding_times, beta, &SYS_Y, &SYS_O, INTEGER(Yvac), length(Yvac), INTEGER(Ovac), length(Ovac));
		} else {
...
struct simresult
seir_model_erlang(double t_phase, double time, double dt, const struct compartments *Y, const struct compartments *O, const double mean_holding_times[2], const double beta[2][2][2], const struct subcompartments *SYS_Y, const struct subcompartments *SYS_O, int *Yv, int Yvl, int *Ov, int Ovl) {
	int steps = 1;
